...
//0:未审核;1:已通过；2：未通过；
	    if("$zhuangtai=='0'"){
			$this->error("您有有一条申请记录正在审核中，请等待审核");
   ...
   if($zhuangtai=="未通过"){
	...
	$emceeRank_month = D('Beandetail')->query('SELECT uid,sum(bean) as total FROM `ss_beandetail` where type="income" and action="getgift" and date_format(FROM_UNIXTIME(addtime),"%m-%Y")=date_format(now(),"%m-%Y") group by uid order by total desc LIMIT 1');
		...
		$emceeRank_month4 = D('Beandetail')->query('SELECT uid,sum(bean) as total FROM `ss_beandetail` where type="income" and action="getgift" and date_format(FROM_UNIXTIME(addtime),"%m-%Y")=date_format(now(),"%m-%Y") group by uid order by total desc LIMIT 1,6');
		$a=0;
		$b=1;
