...
class Page4 {
	...
	protected $config  =	array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'第一页','last'=>'最后一页','theme'=>'%first% %upPage% %nowPage%/%totalPage% %downPage% %end%');
    // 默认分页变量名
    protected $varPage;
    ...
    public function show() {
        ...
        if ($upRow>0){
			...
			$upPage='<span ><a href="javascript:emceenoScrollPage(\''.$upRow.'\');">上一页</a></span>';
        }else{
        ...
        if ($downRow <= $this->totalPages){
			...
			$downPage='<span><a href="javascript:emceenoScrollPage(\''.$downRow.'\')">下一页</a></span>';
        }else{
        ...
        if($nowCoolPage == 1){
			...
			$theFirst = "<span><a href=\"javascript:emceenoScrollPage('1')\" >首页</a></span>";
        }
        if($nowCoolPage == $this->coolPages){
            $nextPage = "";
            $theEnd="<span class=\"ml10\">末页</span>";
        }else{
			...
			$theEnd = "<span class=\"ml10\"><a href=\"javascript:emceenoScrollPage('".$theEndRow."')\" >末页</a></span>";
        }
        // 1 2 3 4 5
        $linkPage = "";
        for($i=1;$i<=$this->rollPage;$i++){
            ...
            if($page!=$this->nowPage){
                if($page<=$this->totalPages){
					...
					$linkPage .= '<a href="javascript:emceenoScrollPage(\''.$page.'\');">'.$page.'</a>';
                }else{
                    break;
                }
            }else{
        ...
        $pageStr	 =	 str_replace(
			...
			array('%first%','%upPage%','%nowPage%/%totalPage%','%downPage%','%end%'),
            array($theFirst,$upPage,$this->nowPage.'/'.$this->totalPages,$downPage,$theEnd),$this->config['theme']);
        return $pageStr;
    }
