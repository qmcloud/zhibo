...
class Page {
    ...
    public function show() {
        ...
        if ($upRow>0){
			...
			$upPage='<span ><a href="javascript:scrollPage(\''.$upRow.'\');">上一页</a></span>';
        }else{
        ...
        if ($downRow <= $this->totalPages){
			...
			$downPage='<span><a href="javascript:scrollPage(\''.$downRow.'\')">下一页</a></span>';
        }else{
        ...
        if($nowCoolPage == 1){
			...
			$theFirst = "<span><a href=\"javascript:scrollPage('1')\" >首页</a></span>";
        }
        if($nowCoolPage == $this->coolPages){
			...
			$theEnd = "<span class=\"ml10\"><a href=\"javascript:scrollPage('".$theEndRow."')\" >末页</a></span>";
        }
        // 1 2 3 4 5
        $linkPage = "";
        for($i=1;$i<=$this->rollPage;$i++){
            ...
            if($page!=$this->nowPage){
                if($page<=$this->totalPages){
					...
					$linkPage .= '<a href="javascript:scrollPage(\''.$page.'\');">'.$page.'</a>';
                }else{
                    break;
                }
            }else{
