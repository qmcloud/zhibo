...
class Page {
	...
	protected $config  =	array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'第一页','last'=>'最后一页','theme'=>'%upPage% %linkPage% %downPage%');
    // 默认分页变量名
    protected $varPage;
    ...
    public function show() {
        ...
        if ($upRow>0){
			...
			$upPage='<a href="javascript:scrollPage(\''.$upRow.'\');">上一页</a>';
        }else{
        ...
        if ($downRow <= $this->totalPages){
			...
			$downPage='<a href="javascript:scrollPage(\''.$downRow.'\')">下一页</a>';
        }else{
