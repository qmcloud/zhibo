...
class IndexAction extends Action {
    function _initialize(){
		...
		if(!strpos($_SERVER["REQUEST_URI"],'login') && !strpos($_SERVER["REQUEST_URI"],'verify') && !strpos($_SERVER["REQUEST_URI"],'logout') && !$_SESSION['payagent'])
		{
	...
	public function dologin()
    {
		...
		if($uid > 0) {
			...
			if(!$userinfo) {
				...
				if($userinfo[0]['payagent'] =='n'){
					$this->error('您不是充值代理身份');
				}
